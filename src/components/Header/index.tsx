...
const Header = (props: {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      ...
      // ✅ Skip fetch if current page is /newresume
      if (pathname !== "/") return;
      ...
      try {
        ...
        if (response.data.profile && response.data.profile.length > 0) {
          ...
          localStorage.setItem(
            "current_resume_id",
            response?.data?.profile?.[0]?.current_resume_id ?? "1",
          );
          localStorage.setItem(
            "start_date",
            response.data.profile[0]["start_date"],
          );
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    // Get today's date in format: Dec 25, 2024
    const date = new Date();
